...
import Irony.Ast.*;
import Irony.Parsing.*;public class AliasNode extends SqlNodeBase implements IAstNodeInit {protected NObject javonetHandle; /**
	 * GetProperty
	 */
            public ColumnRef getAlias (){ try { return new ColumnRef((NObject)javonetHandle.<NObject>get("Alias"));} catch (JavonetException _javonetException) { _javonetException.printStackTrace(); return null;} }public AliasNode (){  super((NObject) null); try {  javonetHandle = Javonet.New("AliasNode"); super.setJavonetHandle(javonetHandle); } catch (JavonetException _javonetException) { _javonetException.printStackTrace(); } }public AliasNode(NObject handle) {super(handle);this.javonetHandle=handle;}public void setJavonetHandle(NObject handle) {
